...
const GSEMaintenance = ({ token, user }) => {
  ...
  // UPDATED: Base64 file upload for Render compatibility
    ...
    <div>
      ...
      <div style={{ backgroundColor: '#d1ecf1', padding: '10px', borderRadius: '5px', marginBottom: '20px', border: '1px solid #bee5eb' }}>
        <p style={{ margin: 0, fontSize: '13px' }}>
          ...
          📎 <strong>Attachments:</strong> Upload PDF, images, documents (stored securely in database)
        </p>
      </div>
      ...
      {/* Attachments Modal with Base64 download link */}
              ...
              <div>
                {attachments.map(att => (
                  <div key={att.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', borderBottom: '1px solid #eee' }}>
                    <div>
                      <a 
                        href={`${API_URL}/api/maintenance-attachment/${att.id}/download`} 
                        target="_blank" 
